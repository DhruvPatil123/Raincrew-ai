...
import { 
  ...
  Download, FileJson, Layers, MapPin, Save, Share2, Star, CheckCircle, HelpCircle, FileText, Upload
} from 'lucide-react';
      ...
      try {
        ...
        if (file.name.endsWith('.json')) {
          ...
          if (Array.isArray(json)) {
            parsedQuestions = json.map(item => typeof item === 'string' ? item : JSON.stringify(item));
          } else if (json.questions && Array.isArray(json.questions)) {
            parsedQuestions = json.questions.map(item => String(item));
          } else {
            throw new Error("JSON should be an array of questions or have a 'questions' array field.");
          }
        ...
        if (parsedQuestions.length === 0) {
          setAiError("No valid questions found in the selected file.");
          return;
        }

        setQuestions(prev => [...prev, ...parsedQuestions]);
        setToastMessage(`Successfully imported ${parsedQuestions.length} questions from ${file.name}!`);
        setTimeout(() => setToastMessage(null), 4000);
      } catch (err: any) {
        setAiError(`File parsing failed: ${err.message || 'Check file format'}`);
      }
    };
    ...
    >
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gradient-to-tr from-slate-900 to-indigo-950 text-white rounded-2xl p-6 border border-slate-800 space-y-4">
          ...
          {/* Manual Input & File Upload fields */}
          <div className="flex flex-col gap-3">
            ...
            {/* Questions File Import Widget */}
            <div className="border border-dashed border-slate-800 rounded-xl p-4 bg-slate-950/40 text-center space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider font-sans flex items-center gap-1.5">
                  <Upload className="w-3.5 h-3.5 text-blue-500" />
                  Import Questions File
                </span>
                <span className="text-[9.5px] font-medium text-slate-500 bg-slate-900 border border-slate-800/60 px-2.5 py-0.5 rounded">
                  JSON / CSV / TXT supported
                </span>
              </div>
              <p className="text-[10px] text-slate-500 text-left">
                Directly parse plain txt (one question per line), CSV columns, or standard JSON question lists.
              </p>
              <div className="relative">
                ...
                <button
                  type="button"
                  className="w-full py-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-1.5"
                >
                  <Upload className="w-3.5 h-3.5 text-slate-400" />
                  Choose File to Import Question Bank
                </button>
              </div>
            </div>
          </div>
