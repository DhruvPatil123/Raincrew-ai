...
import { 
  ...
  ThumbsUp, ThumbsDown, MessageSquare, Copy, ShieldAlert, Expand, Square
} from 'lucide-react';
...
export default function InterviewRoom({ 
  ...
  const PRACTICE_QUESTIONS = [
    "Could you briefly introduce yourself and tell us what you find most exciting about this field?",
    "Describe a challenging problem you solved recently and how you approached the solution."
  ];

  // Active interview state
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [interviewQuestions, setInterviewQuestions] = useState<string[]>(isPractice ? PRACTICE_QUESTIONS : (campaign.questions || []));
  ...
  useEffect(() => {
    if (campaign) {
      if (isPractice) {
        setInterviewQuestions(PRACTICE_QUESTIONS);
      } else if (campaign.questions) {
  ...
  const videoRefCallback = useCallback((el: HTMLVideoElement | null) => {
    ...
    if (el && videoStream) {
      if (el.srcObject !== videoStream) {
        ...
        el.play().catch(e => console.log("Stream play interrupted: ", e));
      }
    }
  }, [videoStream]);
  const [capturedScreenshots, setCapturedScreenshots] = useState<string[]>([]);

  const captureProctorScreenshot = () => {
    try {
      ...
      if (ctx) {
        if (videoRef.current && videoStream) {
          ...
          ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.arc(320, 240, 110, 0, 2 * Math.PI);
          ctx.stroke();
          ...
          ctx.fillText('PROCTOR SECURE SCREEN CAPTURE', 50, 45);
          ctx.font = '12px monospace';
          ctx.fillStyle = '#38bdf8';
          ctx.fillText(`Recorded Time: ${new Date().toLocaleTimeString()} UTC`, 50, 75);
          ctx.fillText(`Target Candidate: ${name || 'Vetting Candidate'}`, 50, 100);
          ctx.fillText('MediaPipe Landmarkers: Coords active', 50, 125);
          ctx.fillStyle = '#4ade80';
          ctx.fillText('Status: Biometric Verification Cleared', 50, 150);
        }
      ...
      console.error("Failed capturing screen capture snapshot:", err);
    }
  };

  const [isManualAnomalyOverride, setIsManualAnomalyOverride] = useState(false);
  ...
  // Live Speech-To-Text Testing States
  const [isTestingSTT, setIsTestingSTT] = useState(false);
  const [testSTTTranscript, setTestSTTTranscript] = useState('');
  const [sttTestStatus, setSttTestStatus] = useState<'Stopped' | 'Connecting...' | 'Listening...' | 'Error'>('Stopped');
  const testWsRef = useRef<WebSocket | null>(null);
  const testScriptProcessorRef = useRef<any>(null);
  const testAudioContextRef = useRef<any>(null);

  const startTestSTT = () => {
    setIsTestingSTT(true);
    setTestSTTTranscript('');
    setSttTestStatus('Connecting...');

    try {
      const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
      const wsUrl = `${protocol}//${window.location.host}/api/transcribe-live`;
      console.log("[STT Test] Connecting Live WebSocket:", wsUrl);

      const ws = new WebSocket(wsUrl);
      testWsRef.current = ws;

      ws.onopen = () => {
        console.log("[STT Test] WebSocket connected successfully.");
        setSttTestStatus('Listening...');
      };
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (message.type === "transcript") {
            ...
            setTestSTTTranscript(rawText);
          }
        } catch (e) {
          console.error("[STT Test] parse error:", e);
        }
      };
      ...
      ws.onclose = () => {
        console.log("[STT Test] closed.");
        setSttTestStatus('Stopped');
      };

      if (micStream) {
        let audioCtx = testAudioContextRef.current;
        ...
        testScriptProcessorRef.current = processor;
      }
    } catch (e) {
      console.error("[STT Test] setup failed:", e);
      setSttTestStatus('Error');
    }
  };
  ...
  const stopTestSTT = () => {
    ...
    if (testScriptProcessorRef.current) {
      try {
        testScriptProcessorRef.current.disconnect();
      } catch (err) {}
      testScriptProcessorRef.current = null;
    }
  ...
  const handleStartInterview = () => {
    ...
    enterFullscreen();
    // Momentary pause to let camera activate/stream bind, then capture setup check
    setTimeout(() => {
      captureProctorScreenshot();
    }, 1505);
  };
    ...
    try {
      if (!activeCandidateId) {
        ...
        const created = addCandidate({
          ...
          name: isPractice ? `[Practice Sandbox] ${name}` : name,
          email: email,
          phone: phone,
          ...
          proctorScreenshots: capturedScreenshots
        });
  ...
  // Stream is bound using videoRefCallback callback ref directly on mount to handle updates and dynamic DOM remounts cleanly.
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (isManualAnomalyOverride) return;
  ...
  }, [phase, campaign.enableMediaPipeCameraProctor, isManualAnomalyOverride]);
  ...
  return (
    <div className="max-w-4xl mx-auto bg-slate-50 border border-slate-100 rounded-3xl overflow-hidden shadow-xl min-h-[500px] flex flex-col">
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[9999] flex flex-col items-center justify-center p-6 text-center text-white">
          ...
          >
            ...
            <button
              ...
              type="button"
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-600/20 active:scale-95 cursor-pointer flex items-center justify-center gap-2"
            >
      ...
      {/* Upper header */}
      <div className="bg-white border-b border-slate-100 px-4 sm:px-8 py-3 sm:py-5 flex items-center justify-between">
        <div>
          <span className="px-2 py-0.5 rounded-full bg-slate-900 text-white font-bold text-[9px] sm:text-[10px] uppercase tracking-widest">Digital Boarding</span>
          <h2 className="text-sm sm:text-md font-extrabold text-slate-800 mt-0.5 sm:mt-1">{campaign.title} Interview</h2>
        </div>
        <button
          onClick={onCancel}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'setup' && (
            ...
            >
              <div className="text-center space-y-3">
                {isPractice ? (
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50 border border-indigo-150 rounded-full text-indigo-700 text-[10px] font-bold uppercase tracking-wider mx-auto font-mono mb-1">
                ...
                <h3 className="text-lg font-display font-extrabold text-slate-800 tracking-tight">
                  {isPractice ? "Mock Sandbox Entry" : "Identify Yourself"}
                </h3>
                <p className="text-xs text-slate-400 font-light leading-relaxed">
                  {isPractice 
                    ? "Confirm your name below to initialize your voice diagnostic test-drive. Results are completely private and never visible to hiring managers."
                    : "Please provide your details below. This will register your candidate screening record and generate your detailed evaluation scorecards."
                  }
                </p>
              </div>
          ...
          {phase === 'ready' && (
            ...
            >
              ...
              {/* Live Speech-to-Text Test Card */}
              <div className="bg-white border border-slate-100 rounded-3xl p-5 space-y-3.5 text-left shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">
                    🎤 Live Speech-to-Text Diagnostics
                  </span>
                  <span className={`text-[9.5px] font-mono font-bold px-2 py-0.5 rounded-full ${
                    sttTestStatus === 'Listening...' ? 'bg-emerald-50 text-emerald-600 animate-pulse' :
                    sttTestStatus === 'Connecting...' ? 'bg-amber-50 text-amber-600' :
                ...
                <div className="flex items-center gap-2.5">
                  {!isTestingSTT ? (
                    <button
                      type="button"
                      onClick={startTestSTT}
                      className="px-3.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-650 text-xs font-bold rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
                    >
                      <Mic className="w-3.5 h-3.5" /> Start Live STT Test
                    </button>
                  ) : (
                    <button
                      type="button"
                      onClick={stopTestSTT}
                      className="px-3.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-bold rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
                    >
                      <Square className="w-3.5 h-3.5 shrink-0" /> Stop STT Test
                    </button>
                  )}
                  <p className="text-[10px] text-slate-400 font-medium">Verify that the AI parses your spoken words verbatim</p>
                </div>

                {isTestingSTT && (
                  <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 text-left min-h-[60px] flex flex-col justify-between shadow-inner">
                    <p className={`text-xs font-mono leading-relaxed tracking-tight ${testSTTTranscript ? 'text-slate-100' : 'text-slate-500 italic'}`}>
                      {testSTTTranscript || 'Speak clearly into your microphone now... Transcribed text will appear here.'}
                    </p>
                    <span className="text-[8px] font-mono text-slate-500 mt-2.5 block select-none">
                      Connected to /api/transcribe-live • Powered by Gemini AI
                    </span>
                  </div>
                )}
              </div>
          ...
          {phase === 'interviewing' && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 max-w-4xl mx-auto w-full items-stretch"
            >
              {/* Question board left */}
              <div className="md:col-span-1 bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-4 sm:p-6 flex flex-col justify-between shadow-sm">
                <div className="space-y-3 sm:space-y-4">
                  {isPractice && (
                    <div className="bg-indigo-50 border border-indigo-100/60 p-3 rounded-xl sm:rounded-2xl text-left font-sans">
                      <p className="font-extrabold text-[11px] text-indigo-900 leading-tight flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
                        Warmup Sandbox
                      </p>
                      <p className="text-[9.5px] text-indigo-750 font-medium leading-normal pt-0.5">
                        Confidence trial is completely private. Tests audio capture & vocal metrics.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-blue-600">
                      {isPractice ? "Mock Sandbox" : "Active Stage"}
                    </span>
                  ...
                  <div className="space-y-2">
                    {isFollowUpTurn ? (
                      <div className="space-y-1.5 bg-blue-50/70 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-blue-100/80">
                        <span className="text-[9px] sm:text-[9.5px] font-extrabold uppercase tracking-widest text-indigo-750 bg-indigo-100/60 px-2 py-0.5 rounded-full flex items-center gap-1 w-max">
                          <Sparkles className="w-3 h-3 text-indigo-600 fill-indigo-200 animate-pulse" />
                          Adaptive follow-up
                        </span>
                        <p className="text-slate-800 text-[12px] sm:text-[12.5px] font-extrabold italic leading-relaxed">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-800 text-xs sm:text-sm font-extrabold leading-relaxed">
                  ...
                  {/* Active STT Engine Stack Display - hidden on mobile to prioritize screen space */}
                  <div className="mt-3 bg-slate-50 border border-slate-100 p-2.5 rounded-xl text-[10px] text-slate-500 space-y-1 hidden md:block">
                  ...
                  {campaign.enableMediaPipeCameraProctor !== false && (
                    <div className="mt-2.5 sm:mt-3.5 space-y-2 border-t border-slate-150 pt-2.5 sm:pt-3 flex-1 flex flex-col justify-end">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-rose-600 flex items-center gap-1 font-sans">
                      ...
                      <div className="relative aspect-video max-w-[220px] sm:max-w-none mx-auto w-full rounded-xl sm:rounded-2xl bg-slate-950 overflow-hidden border border-slate-100 shadow-inner flex items-center justify-center">
                        {videoStream ? (
                          <video
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 p-3 text-center">
                            <User className="w-6 h-6 opacity-40 mb-1" />
                            <p className="text-[8.5px] font-medium leading-relaxed">No camera allowed. Simulated geometric proctor fallback tracking coordinates.</p>
                          </div>
                        )}
                      ...
                      <div className="grid grid-cols-2 text-[9px] gap-2 font-mono text-slate-500 bg-slate-50 p-2 rounded-xl transition-all font-sans">
                        <div className="flex justify-between border-r border-slate-200/60 pr-2">
                          <span>Head Pitch:</span>
                          <span className="font-bold text-slate-700 font-mono">{pitchEuler}°</span>
                        </div>
                        <div className="flex justify-between pl-1">
                          <span>Head Yaw:</span>
                          <span className="font-bold text-slate-700 font-mono">{yawEuler}°</span>
                        </div>
                      </div>
                    </div>
                  )}

                  {/* Developer Integrity Diagnostic Anomaly Simulator - hidden on mobile */}
                  <div className="mt-3.5 space-y-2 border-t border-slate-150 pt-3 bg-slate-50/50 p-3 rounded-2xl border border-slate-100/60 text-left hidden md:block">
                    <span className="text-[9.5px] font-mono font-bold uppercase tracking-widest text-[#0ea5e9] flex items-center gap-1.5 select-none">
                      <ShieldAlert className="w-3.5 h-3.5 text-[#0ea5e9]" />
                      Developer Proctor Testing Hub
                    </span>
                    <p className="text-[9px] text-slate-400 leading-normal">
                      Simulate candidate integrity violations or camera anomalies to test real-time proctor logs and scorecards dynamically.
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-1.5 pt-1">
                      ...
                      <button
                        onClick={() => {
                          setTabSwitchCount(prev => {
                            const updated = prev + 1;
                            console.warn(`[DEVELOPER SIMULATOR] Tab switch simulation trigger. Total: ${updated}`);
                            setSuspiciousVoiceNoise(true);
                            setShowIntegrityWarning(true);
                            setIsFullscreen(false); // Triggers the full-screen warning lockout!
                            return updated;
                          });
                        }}
                        type="button"
                        className="p-2 text-center bg-amber-50 hover:bg-amber-100 rounded-xl text-[10.5px] font-bold text-amber-700 hover:text-amber-800 transition-all border border-amber-105 cursor-pointer active:scale-95"
                      >
                        🗂️ Simulate Tab Switch
                      </button>

                      {/* Sim Screen Exit Fullscreen */}
                      <button
                        onClick={() => {
                          setFullscreenExitCount(prev => {
                            const updated = prev + 1;
                            console.warn(`[DEVELOPER SIMULATOR] Fullscreen exit simulation trigger. Total: ${updated}`);
                            setSuspiciousVoiceNoise(true);
                            setShowIntegrityWarning(true);
                        ...
                        type="button"
                        className="p-2 text-center bg-rose-50 hover:bg-rose-100 rounded-xl text-[10.5px] font-bold text-rose-700 hover:text-rose-800 transition-all border border-rose-105 cursor-pointer active:scale-95"
                      >
                        🖥️ Sim Fullscreen Exit
                      </button>

                      {/* Sim Multiple Faces */}
                      <button
                        onClick={() => {
                          setIsManualAnomalyOverride(true);
                          setFacesDetected(2);
                          setGazeDirection('Center');
                          console.warn(`[DEVELOPER SIMULATOR] Multiple faces anomaly injected.`);
                        }}
                        type="button"
                        className={`p-2 text-center rounded-xl text-[10.5px] font-bold border transition-all cursor-pointer active:scale-95 ${
                          isManualAnomalyOverride && facesDetected === 2 
                      ...
                      <button
                        onClick={() => {
                          ...
                          setFacesDetected(0);
                          setGazeDirection('Away');
                          console.warn(`[DEVELOPER SIMULATOR] No face anomaly injected.`);
                        }}
                        type="button"
                        className={`p-2 text-center rounded-xl text-[10.5px] font-bold border transition-all cursor-pointer active:scale-95 ${
                          isManualAnomalyOverride && facesDetected === 0
                    ...
                    <div className="flex gap-2.5 pt-1.5 items-center justify-between border-t border-slate-100 mt-2">
                      <span className="text-[8px] font-mono text-slate-400">
                        Status: {isManualAnomalyOverride ? '🔬 Manual Simulate Active' : '🤖 Live Capture Loop'}
                      </span>
                      {isManualAnomalyOverride && (
                        <button
                          onClick={() => {
                            setIsManualAnomalyOverride(false);
                            setFacesDetected(1);
                            setGazeDirection('Center');
                            console.log(`[DEVELOPER SIMULATOR] Anomaly simulation reset to automatic.`);
                          }}
                          type="button"
                          className="px-2 py-0.5 text-[8.5px] font-bold uppercase tracking-wider text-indigo-650 hover:text-indigo-805 cursor-pointer bg-slate-100 rounded-md"
                        >
                          🔄 Reset To Auto
                        </button>
                      )}
                ...
                <div className="text-xs text-slate-400 border-t border-slate-50 pt-4 leading-normal mt-4 hidden sm:block">
