...
import { 
  ...
  Send, Palette, History, ShieldAlert, BarChart3, AudioLines, Database
} from 'lucide-react';
...
export default function Dashboard({ 
  ...
  const [supUrlInput, setSupUrlInput] = useState('');
  const [supKeyInput, setSupKeyInput] = useState('');
  ...
  const loadDbStatus = () => {
    fetch('/api/db-status')
      ...
      .then(data => {
        setDbStatus(data);
        if (data.supabaseUrl && !supUrlInput) {
          setSupUrlInput(data.supabaseUrl);
        }
      })
      .catch(console.error);
  };
...
${recruiterCompany}`;
  ...
  const handleSaveSupabaseConfig = async () => {
    ...
    try {
      const res = await fetch('/api/save-supabase-config', {
        ...
        body: JSON.stringify({
          supabaseUrl: supUrlInput,
          supabaseServiceKey: supKeyInput
        })
      });
      const data = await res.json();
      if (res.ok) {
        setDbStatusMsg({ type: 'success', text: data.message || 'Supabase config applied successfully!' });
        loadDbStatus();
        setSupKeyInput(''); // Clear masked password key
      } else {
        setDbStatusMsg({ type: 'error', text: data.error || 'Failed to save config.' });
      }
    } catch (err: any) {
      setDbStatusMsg({ type: 'error', text: 'Connection refused/network error.' });
    } finally {
      setIsSavingDb(false);
    }
  };
  ...
  const handleClearSupabaseConfig = async () => {
    ...
    setDbStatusMsg(null);
    try {
      const res = await fetch('/api/clear-supabase-config', { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
        setDbStatusMsg({ type: 'success', text: data.message || 'Fell back to local JSON database successfully.' });
        setSupUrlInput('');
        setSupKeyInput('');
        loadDbStatus();
      } else {
        setDbStatusMsg({ type: 'error', text: data.error || 'Failed to clear config.' });
      }
    } catch (err: any) {
      setDbStatusMsg({ type: 'error', text: 'Network connection error.' });
    } finally {
      setIsSavingDb(false);
    }
  };
  ...
  const filteredCandidates = candidates.filter(cand => {
    if (cand.isPractice) return false;
      ...
      {activeTab === 'branding' && (
        ...
        >
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Supabase Database Connection Status Indicator Widget */}
            <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-slate-50 pb-3">
                <div className="flex items-center gap-2">
                  <Database className="w-4.5 h-4.5 text-indigo-600" />
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800 font-mono">Supabase PostgreSQL Driver</h3>
                </div>
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[9px] font-extrabold uppercase rounded-full tracking-wider ${
                  dbStatus.activeDbProvider === 'supabase'
                    ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                    : 'bg-indigo-50 text-indigo-700 border border-indigo-200'
                }`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${
                    dbStatus.activeDbProvider === 'supabase' ? 'bg-emerald-500 animate-pulse' : 'bg-indigo-500'
                  }`} />
                  {dbStatus.activeDbProvider === 'supabase' ? 'Supabase Active' : 'Local Active'}
                </span>
              </div>
              ...
              <div className="space-y-3">
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-500 uppercase font-mono">Connection Diagnostics:</span>
                    <span className={`px-1.5 py-0.5 text-[8.5px] font-extrabold uppercase rounded font-mono ${
                      dbStatus.supabaseStatus === 'ok' ? 'bg-emerald-100 text-emerald-800' :
                      dbStatus.supabaseStatus === 'tables_missing' ? 'bg-amber-100 text-amber-800 animate-pulse' :
                      dbStatus.supabaseStatus === 'error' ? 'bg-rose-100 text-rose-800' :
                      'bg-slate-200 text-slate-700'
                    }`}>
                      {dbStatus.supabaseStatus === 'ok' ? 'Online & Linked' :
                       dbStatus.supabaseStatus === 'tables_missing' ? 'SQL Tables Missing' :
                       dbStatus.supabaseStatus === 'error' ? 'Connection Error' :
                       'Not Configured'}
                    </span>
                  </div>

                  {dbStatus.supabaseStatus === 'tables_missing' && (
                    <p className="text-[10.5px] text-amber-800 leading-normal font-semibold">
                      ⚠ Connected successfully, but your database tables don't exist yet. Cut & paste the schema below in Supabase SQL editor to launch!
                    </p>
                  )}
                  {dbStatus.supabaseError && (
                    <p className="text-[10px] text-rose-700 font-mono break-words leading-tight">
                      Error: {dbStatus.supabaseError}
                    </p>
                  )}
                  {dbStatus.supabaseUrl && (
                    <div className="text-[10px] text-slate-500 font-mono flex items-center justify-between">
                      <span>Endpoint:</span>
                      <span className="text-slate-800 font-semibold">{dbStatus.supabaseUrl}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider font-mono">Supabase Reference URL</label>
                    <input
                      type="text"
                      placeholder="https://yourproject.supabase.co"
                      value={supUrlInput}
                      onChange={(e) => setSupUrlInput(e.target.value)}
                      className="w-full text-xs font-mono px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-50 focus:border-indigo-500 outline-none"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider font-mono">Service Role API Key</label>
                    <input
                      type="password"
                ...
                {dbStatusMsg && (
                  <div className={`p-2 rounded-xl text-[10px] text-center font-bold ${
                    dbStatusMsg.type === 'success' ? 'bg-emerald-50 text-emerald-800 border border-emerald-110' : 'bg-rose-50 text-rose-800 border border-rose-110'
                  }`}>
                    {dbStatusMsg.text}
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold rounded-xl text-[10.5px] font-mono transition-colors cursor-pointer uppercase tracking-wider text-center"
                  >
                    {isSavingDb ? 'Linking...' : 'Save Keys'}
                  </button>
                  <button
                    type="button"
                    disabled={isSavingDb}
                    onClick={handleClearSupabaseConfig}
                    className="py-2 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-600 font-bold rounded-xl text-[10.5px] font-mono transition-colors cursor-pointer uppercase tracking-wider text-center"
                  >
                    Clear Keys
                  </button>
                </div>
                ...
                {dbStatus.sqlSchema && (
                  <div className="space-y-1 pt-1 border-t border-slate-100">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-bold text-indigo-600 uppercase font-mono">Initialization SQL Schema:</span>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="text-[9.5px] hover:underline hover:text-indigo-600 text-indigo-700 font-extrabold uppercase font-mono"
                      >
                        Copy SQL
                      </button>
                    </div>
                    <pre className="text-[9px] bg-slate-900 text-slate-300 font-mono p-2 rounded-lg max-h-36 overflow-y-auto whitespace-pre leading-relaxed select-all">
                      {dbStatus.sqlSchema}
                    </pre>
                  </div>
                )}
