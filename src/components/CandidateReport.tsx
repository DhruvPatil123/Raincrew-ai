          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            {gdprReceipt ? (
              ...
              <div className="space-y-3 text-xs text-slate-600">
                ...
                {/* Captured Screenshots Proctor check */}
                {candidate.proctorScreenshots && candidate.proctorScreenshots.length > 0 && (
                  <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <div className="flex items-center justify-between">
                      <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider flex items-center gap-1">
                        ...
                        Proctor Verification Snapshots ({candidate.proctorScreenshots.length})
                      </p>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {candidate.proctorScreenshots.map((shot, sIdx) => (
                        <div key={sIdx} className="group relative rounded-lg overflow-hidden border border-slate-200 bg-slate-950 aspect-video shadow-xs transition-transform hover:scale-102">
                          <img 
                            src={shot} 
                            alt={`Proctor Verification Capture ${sIdx + 1}`} 
                            referrerPolicy="no-referrer"
                            className="w-full h-full object-cover"
                          />
                          <div className="absolute bottom-0 inset-x-0 bg-slate-900/80 p-1 text-[8.5px] text-slate-300 font-mono text-center flex items-center justify-center gap-1 backdrop-blur-sm">
                    ...
                    <p className="text-[9px] text-slate-400 italic leading-relaxed">
                      Screenshots are encrypted in browser storage databases securely, tracking orientation, coordinates, and compliance vectors.
                    </p>
                  </div>
                )}
