...
interface CandidateDashboardProps {
  ...
  onStartInterview: (campaign: Campaign, candidateId?: string, isPractice?: boolean) => void;
  onLogout: () => void;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="space-y-4">
            ...
            {/* Mock Interview Practice sandbox banner */}
            <div className="bg-gradient-to-r from-indigo-900 to-indigo-950 text-white rounded-3xl p-6 relative overflow-hidden border border-indigo-850 shadow-md">
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.06)_0,transparent_45%)] pointer-events-none" />
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10 text-left">
                <div className="space-y-1">
                  <span className="text-[8.5px] font-mono leading-none bg-indigo-500/25 text-indigo-300 px-2.5 py-1 rounded-full uppercase font-black border border-indigo-500/20">
                    🧑💻 Oral Warmup Sandbox
                  </span>
                  <h4 className="text-sm font-extrabold text-white">Anxious? Take an AI Practice Run</h4>
                  <p className="text-[11px] text-slate-300 leading-relaxed max-w-xl">
                    Check your microphone reactivity and run a 2-question vocal test-drive. Get instant capability scorecard reviews. Practice results are completely private and never shown to recruiters.
                  </p>
                </div>
                  ...
                  <button
                    onClick={() => {
                      if (campaigns.length > 0) {
                        onStartInterview(campaigns[0], undefined, true);
                      }
                    }}
                    type="button"
                    className="w-full sm:w-auto px-4 py-2 bg-white hover:bg-slate-100 text-indigo-950 font-extrabold text-xs rounded-xl shadow-sm hover:shadow transition-all uppercase flex items-center justify-center gap-1.5 cursor-pointer active:scale-95"
                  >
                    Launch Mock Sandbox 🧑💻
                  </button>
                </div>
                      ...
                      {hasEvaluated ? (
                        <div className="flex items-center justify-between w-full">
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              type="button"
                              onClick={() => onStartInterview(camp, undefined, false)}
                          ...
                          <button
                            type="button; "
                            onClick={() => onStartInterview(camp, undefined, true)}
                            className="text-[9.5px] font-extrabold text-indigo-600 bg-indigo-50/70 hover:bg-indigo-100 hover:text-indigo-700 border border-indigo-100/30 px-2.5 py-1.5 rounded-xl transition-all cursor-pointer uppercase"
                          >
                            Practice Mode
                          </button>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 w-full">
                          <button
                            ...
                            onClick={() => {
                              ...
                              onStartInterview(camp, pendingObj?.id, false);
                            }}
                          ...
                          <button
                            type="button"
                            onClick={() => onStartInterview(camp, undefined, true)}
                            className="py-2 px-3 bg-indigo-50 hover:bg-indigo-100 text-indigo-750 border border-indigo-100/30 hover:border-indigo-200 font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 leading-none uppercase"
                          >
                            🧑💻 Practice
                          </button>
                        </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {mySubmissions.map((sub) => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-3.5">
                            <span className="font-extrabold text-slate-800 group-hover:text-blue-600 transition-colors flex items-center gap-2">
                              {associatedCamp ? associatedCamp.title : 'Legacy Custom Vetting Role'}
                              {sub.isPractice && (
                                <span className="text-[8px] tracking-wider font-extrabold font-mono leading-none bg-indigo-50 border border-indigo-100 text-indigo-650 px-1.5 py-0.5 rounded-md uppercase">
                                  Mock Round
                                </span>
                              )}
                            </span>
                          ...
                          <td className="py-3.5">
                            <div className="flex items-center gap-1.5">
                              {sub.isPractice ? (
                                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-650 bg-indigo-50 border border-indigo-100 px-2.5 py-0.5 rounded-full select-none">
                                  ✓ Warmup Run
                                </span>
