...
import { initAuth, getSupabaseClient } from './lib/googleAuth';
...
export default function App() {
  ...
  const [currentUserRole, setCurrentUserRole] = useState<'recruiter' | 'candidate' | null>(null);
  ...
  const [isInterviewPracticeMode, setIsInterviewPracticeMode] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    // If we are currently inside an OAuth popup window, wait for standard session processing, then close!
    if (window.opener && (window.location.hash.includes('access_token=') || window.location.search.includes('code='))) {
      setTimeout(() => {
        try {
          ...
          console.warn("Could not post message back to main window", e);
        }
        window.close();
      }, 1200);
      return;
    ...
    // Initialize google / github live auth listener
    const unsubscribeAuth = initAuth(
      (user, token) => {
        const finalSession = {
          email: user.email || 'recruiter@foloup.ai',
          name: user.displayName || 'Alexander Recruiter',
          provider: (user.provider || 'google') as 'google' | 'github' | 'magic-link',
          organization: user.organization || 'Raincrew.AI Workspace',
          workspaceId: user.email ? user.email.split('@')[1].toLowerCase() : 'sandbox',
          token: token
        };
      ...
      () => {
        const savedSession = localStorage.getItem('foloup_recruiter_session');
        if (savedSession) {
          try {
            ...
            console.warn("Failed to parse saved recruiter session", e);
          }
        }
      }
    );
    ...
    const handleOauthMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'SUPABASE_OAUTH_SUCCESS') {
        getSupabaseClient().then(client => {
          if (client) {
            client.auth.getSession().then(({ data: { session: currSession } }: any) => {
              if (currSession && currSession.user) {
                const user = currSession.user;
                const fallbackName = user.user_metadata?.full_name || user.email?.split('@')[0] || "User";
                const metaOrg = user.user_metadata?.organization || (user.email ? user.email.split('@')[1].split('.')[0].toUpperCase() + " Workspace" : "Raincrew.AI Workspace");
                
                const sessionPayload: RecruiterSession = {
                  email: user.email || 'recruiter@foloup.ai',
                  name: fallbackName,
                  provider: (user.app_metadata?.provider || 'google') as 'google' | 'github' | 'magic-link',
                  organization: metaOrg,
                  workspaceId: user.email ? user.email.split('@')[1].toLowerCase() : 'sandbox',
                  token: currSession.access_token || 'oauth-token'
                };
                
                // Set standard Google auth user storage for complete system parity
                localStorage.setItem('foloup_google_user', JSON.stringify({
                  uid: user.id,
                ...
                localStorage.setItem('foloup_google_token', currSession.access_token || "supabase-token");

                handleSelectRole('recruiter', undefined, undefined, undefined, sessionPayload);
                reloadData();
              }
            });
          }
    ...
    // Dynamic direct session restore on mount
    getSupabaseClient().then(client => {
      if (client) {
        client.auth.getSession().then(({ data: { session: currSession } }: any) => {
          if (currSession && currSession.user) {
  ...
  const handleLaunchCampaignInterview = (campaign: Campaign, candidateId?: string, isPractice = false) => {
    setInterviewCampaign(campaign);
    setInterviewCandidateId(candidateId);
    setIsInterviewPracticeMode(isPractice);
    setView('interview');
  };
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col antialiased transition-colors duration-500 ${
      darkMode 
        ? 'bg-[#050508] text-slate-100 dark' 
        : 'bg-[#fcfcfd] text-slate-800'
    }`}>
      
      {/* Neo-Minimalist Header */}
      <header className={`border-b backdrop-blur-xl py-4 px-6 md:px-8 sticky top-0 z-40 transition-all duration-300 ${
        darkMode 
          ? 'bg-[#0a0a12]/80 border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.5)]' 
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          >
            ...
            <span className={`text-2xl font-display font-black tracking-tighter ${
              ...
              {branding.appName ? (
                branding.appName === 'Raincrew.AI' ? (
                  <>Raincrew<span className="text-indigo-500">.AI</span></>
                ) : (
                  <span className={`bg-clip-text text-transparent bg-gradient-to-r ${
                    darkMode ? 'from-white to-slate-400' : 'from-slate-900 to-indigo-600'
                  }`}>
                    {branding.appName}
                  </span>
                )
            ...
            <span className="text-[9px] font-bold tracking-widest text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full uppercase ml-2 select-none md:inline hidden font-display">V4 Engine</span>
          </div>
              ...
              {view === 'dashboard' && currentUserRole === 'recruiter' && (
                ...
                >
                  <Dashboard
                    campaigns={campaigns}
                    stats={stats}
                    candidates={candidates.filter(c => !c.isPractice)}
              ...
              {view === 'details' && selectedCampaign && currentUserRole === 'recruiter' && (
                ...
                >
                  <CampaignDetails
                    ...
                    candidates={candidates.filter(c => c.campaignId === selectedCampaign.id && !c.isPractice)}
                    onBack={handleBackToDashboard}
                    onOpenReport={(cand) => {
              ...
              {view === 'candidate-dashboard' && currentUserRole === 'candidate' && (
                ...
                >
                  <CandidateDashboard
                    ...
                    onStartInterview={(camp, candId, isPractice) => {
                      handleLaunchCampaignInterview(camp, candId, isPractice);
                    }}
                    onLogout={() => {
                      setCurrentUserRole(null);
                      setView('dashboard');
              ...
              {view === 'interview' && interviewCampaign && (
                ...
                >
                  <InterviewRoom
                    ...
                    isPractice={isInterviewPracticeMode}
