...
// ==========================================
// 📡 DATABASE DIRECTORY & FILE INITIALIZATION (LOCAL BACKUP)
// ==========================================

function loadLocalDatabase(): DBStore {
  try {
    ...
    console.error("Failed to read local database file, using in-memory:", err);
  }
  // Try to write starting seed if none exists
  try {
    ...
    console.error("Failed to write initial local database config:", err);
  }
  return INITIAL_DATABASE;
}

function saveLocalDatabase(store: DBStore): void {
  try {
    const dir = path.dirname(DB_FILE);
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
...
// Dry trigger on build/start
loadLocalDatabase();

// ==========================================
// 🔌 DYNAMIC DATABASE DRIVER SWITCHBOARD (LOCAL vs SUPABASE)
// ==========================================
let activeDbProvider: "local" | "supabase" = "local";
let supabaseConfig: { supabaseUrl?: string; supabaseServiceKey?: string } = {};

try {
  const sfPath = path.join(process.cwd(), "supabase-applet-config.json");
  if (fs.existsSync(sfPath)) {
    const raw = fs.readFileSync(sfPath, "utf-8");
    supabaseConfig = JSON.parse(raw);
  }
} catch (err) {
  console.log("No supabase-applet-config.json configuration parsed.");
}

const supabaseUrl = process.env.SUPABASE_URL || supabaseConfig.supabaseUrl || "";
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY || supabaseConfig.supabaseServiceKey || "";
...
if (supabaseUrl && supabaseServiceKey) {
  try {
    ...
    console.log(`Supabase connection detected! Active database provider: SUPABASE (${supabaseUrl})`);
  } catch (err) {
    console.error("Failed to initialize Supabase client connector:", err);
  }
} else {
  console.log("Supabase parameters not set. Active database provider: LOCAL JSON-FILE");
}

// Supabase CRUD implementations
async function getSupabaseCampaigns(workspaceId: string = "sandbox"): Promise<any[]> {
  try {
    if (!supabase) return [];
    let q = supabase.from('campaigns').select('data');
    ...
    if (error) {
      if (error.code === '42P01') {
        console.warn("Supabase warning: campaigns table not found. Seed placeholder data.");
        return INITIAL_DATABASE.campaigns;
      }
      throw error;
    }
    const list = (data || []).map((val: any) => val.data);
    return list.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  } catch (err) {
    console.error("Failed to query campaigns in Supabase:", err);
    return INITIAL_DATABASE.campaigns;
  }
}

async function saveSupabaseCampaign(campaign: any, workspaceId: string = "sandbox"): Promise<void> {
  try {
    if (!supabase) return;
    ...
    console.error("Failed to upsert campaign in Supabase:", err);
  }
}

async function deleteSupabaseCampaign(campId: string): Promise<void> {
  try {
    if (!supabase) return;
    await supabase.from('campaigns').delete().eq('id', campId);
    await supabase.from('candidates').delete().eq('campaign_id', campId);
  } catch (err) {
    console.error("Failed to delete campaign in Supabase:", err);
  }
}

async function getSupabaseCandidates(campaignId?: string, workspaceId: string = "sandbox"): Promise<any[]> {
  try {
    if (!supabase) return [];
    let q = supabase.from('candidates').select('data');
    if (campaignId) {
    ...
    if (error) {
      if (error.code === '42P01') {
        console.warn("Supabase warning: candidates table not found. Seed placeholder data.");
        return INITIAL_DATABASE.candidates;
      }
      throw error;
    }
    const list = (data || []).map((val: any) => val.data);
    return list.filter((cand: any) => {
      if (workspaceId !== "sandbox" && cand.workspaceId !== workspaceId) return false;
      if (workspaceId === "sandbox" && cand.workspaceId && cand.workspaceId !== "sandbox") return false;
      return true;
    });
  } catch (err) {
    console.error("Failed to query candidates in Supabase:", err);
    return INITIAL_DATABASE.candidates;
  }
}

async function saveSupabaseCandidate(candidate: any, workspaceId: string = "sandbox"): Promise<void> {
  try {
    if (!supabase) return;
    ...
    console.error("Failed to save candidate in Supabase:", err);
  }
}

async function deleteSupabaseCandidate(id: string): Promise<void> {
  try {
    if (!supabase) return;
...
async function getSupabaseAuditLogs(workspaceId: string = "sandbox"): Promise<any[]> {
  try {
    if (!supabase) return [];
    let q = supabase.from('audit_logs').select('data');
    if (workspaceId !== "sandbox") {
      q = q.eq('workspace_id', workspaceId);
    }
    const { data, error } = await q;
    if (error) {
      if (error.code === '42P01') {
        console.warn("Supabase warning: audit_logs table not found. Seed placeholder data.");
        return INITIAL_DATABASE.auditLogs;
      }
      throw error;
    }
    const list = (data || []).map((val: any) => val.data);
    ...
    console.error("Failed to query audit logs in Supabase:", err);
    return INITIAL_DATABASE.auditLogs;
  }
}

async function saveSupabaseAuditLog(log: any, workspaceId: string = "sandbox"): Promise<void> {
  try {
    if (!supabase) return;
    ...
    console.error("Failed to save audit log in Supabase:", err);
  }
}

async function getSupabaseBranding(workspaceId: string = "sandbox"): Promise<any> {
  try {
    if (!supabase) return INITIAL_DATABASE.branding;
    const { data, error } = await supabase.from('branding').select('data').eq('workspace_id', workspaceId).maybeSingle();
    if (error) {
      if (error.code === '42P01') {
        console.warn("Supabase warning: branding table not found. Defaulting to local branding.");
    ...
    console.error("Failed to get branding in Supabase:", err);
    return INITIAL_DATABASE.branding;
  }
}

async function saveSupabaseBranding(branding: any, workspaceId: string = "sandbox"): Promise<void> {
  try {
    if (!supabase) return;
    ...
    console.error("Failed to save branding in Supabase:", err);
  }
}

async function getSupabaseRecruiterNotes(candidateId: string): Promise<string[]> {
  try {
    if (!supabase) return [];
    const { data, error } = await supabase.from('recruiter_notes').select('notes').eq('candidate_id', candidateId).maybeSingle();
    if (error) {
    ...
    console.error("Failed to read recruiter notes from Supabase:", err);
    return [];
  }
}

async function saveSupabaseRecruiterNote(candidateId: string, note: string): Promise<string[]> {
  try {
    if (!supabase) return [];
    const notes = await getSupabaseRecruiterNotes(candidateId);
    notes.push(note);
    const { error } = await supabase.from('recruiter_notes').upsert({
    ...
    console.error("Failed to save recruiter note in Supabase:", err);
    return [];
  }
}

async function getSupabaseEmailLogs(workspaceId: string = "sandbox"): Promise<any[]> {
  try {
    if (!supabase) return [];
    let q = supabase.from('email_logs').select('data');
    if (workspaceId !== "sandbox") {
      q = q.eq('workspace_id', workspaceId);
    }
    const { data, error } = await q;
    ...
    return (data || []).map((val: any) => val.data);
  } catch (err) {
    console.error("Failed to fetch email logs in Supabase:", err);
    return [];
  }
}

async function saveSupabaseEmailLog(email: any, workspaceId: string = "sandbox"): Promise<void> {
  try {
    if (!supabase) return;
    ...
    console.error("Failed to save email log in Supabase:", err);
  }
}


// ==========================================
// 🔌 DYNAMIC BACKEND DATABASE MANAGER (SUPABASE OR LOCAL)
// ==========================================

async function getDatabaseCampaigns(workspaceId: string = "sandbox"): Promise<any[]> {
  if (activeDbProvider === "supabase") {
    return getSupabaseCampaigns(workspaceId);
  }
  const store = loadLocalDatabase();
  return (store.campaigns || []).filter(c => {
    if (workspaceId === "sandbox") {
      return !c.workspaceId || c.workspaceId === "sandbox";
    }
    return c.workspaceId === workspaceId;
  }).sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}

async function saveDatabaseCampaign(campaign: any, workspaceId: string = "sandbox"): Promise<void> {
  if (activeDbProvider === "supabase") {
    return saveSupabaseCampaign(campaign, workspaceId);
  }
  const store = loadLocalDatabase();
  if (!store.campaigns) store.campaigns = [];
  const index = store.campaigns.findIndex(c => c.id === campaign.id);
  const dataToSave = { ...campaign, workspaceId };
  if (index >= 0) {
    store.campaigns[index] = dataToSave;
  } else {
    store.campaigns.push(dataToSave);
  }
  saveLocalDatabase(store);
}

async function deleteDatabaseCampaign(campId: string): Promise<void> {
  if (activeDbProvider === "supabase") {
    return deleteSupabaseCampaign(campId);
  }
  const store = loadLocalDatabase();
  store.campaigns = (store.campaigns || []).filter(c => c.id !== campId);
  store.candidates = (store.candidates || []).filter(c => c.campaignId !== campId);
  saveLocalDatabase(store);
}

async function getDatabaseCandidates(campaignId?: string, workspaceId: string = "sandbox"): Promise<any[]> {
  if (activeDbProvider === "supabase") {
    return getSupabaseCandidates(campaignId, workspaceId);
  }
  const store = loadLocalDatabase();
...
async function saveDatabaseCandidate(candidate: any, workspaceId: string = "sandbox"): Promise<void> {
  if (activeDbProvider === "supabase") {
    return saveSupabaseCandidate(candidate, workspaceId);
  }
  const store = loadLocalDatabase();
  if (!store.candidates) store.candidates = [];
  const index = store.candidates.findIndex(c => c.id === candidate.id);
  const dataToSave = { ...candidate, workspaceId };
  if (index >= 0) {
    store.candidates[index] = dataToSave;
  } else {
    store.candidates.push(dataToSave);
  }
  saveLocalDatabase(store);
}

async function deleteDatabaseCandidate(id: string): Promise<void> {
  if (activeDbProvider === "supabase") {
    return deleteSupabaseCandidate(id);
  }
  const store = loadLocalDatabase();
...
async function getDatabaseAuditLogs(workspaceId: string = "sandbox"): Promise<any[]> {
  if (activeDbProvider === "supabase") {
    return getSupabaseAuditLogs(workspaceId);
  }
  const store = loadLocalDatabase();
  return (store.auditLogs || []).filter(log => {
    if (workspaceId === "sandbox") {
      return !log.workspaceId || log.workspaceId === "sandbox";
    }
    return log.workspaceId === workspaceId;
  }).sort((a: any, b: any) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
}

async function saveDatabaseAuditLog(log: any, workspaceId: string = "sandbox"): Promise<void> {
  if (activeDbProvider === "supabase") {
    return saveSupabaseAuditLog(log, workspaceId);
  }
  const store = loadLocalDatabase();
  if (!store.auditLogs) store.auditLogs = [];
  const index = store.auditLogs.findIndex(l => l.id === log.id);
  const dataToSave = { ...log, workspaceId };
  if (index >= 0) {
    store.auditLogs[index] = dataToSave;
  } else {
    store.auditLogs.push(dataToSave);
  }
  saveLocalDatabase(store);
}

async function getDatabaseBranding(workspaceId: string = "sandbox"): Promise<any> {
  if (activeDbProvider === "supabase") {
    return getSupabaseBranding(workspaceId);
  }
  const store = loadLocalDatabase();
  if (!store.branding) {
    store.branding = {};
  }
  if (store.branding[workspaceId]) {
    return store.branding[workspaceId];
  }
  return {
    ...INITIAL_DATABASE.branding,
    companyName: workspaceId === "sandbox" ? "Raincrew.AI" : workspaceId.split('.')[0].toUpperCase() + " Workspace"
...
async function saveDatabaseBranding(branding: any, workspaceId: string = "sandbox"): Promise<void> {
  if (activeDbProvider === "supabase") {
    return saveSupabaseBranding(branding, workspaceId);
  }
  const store = loadLocalDatabase();
  if (!store.branding) {
    store.branding = {};
  }
  store.branding[workspaceId] = branding;
  saveLocalDatabase(store);
}

async function getDatabaseRecruiterNotes(candidateId: string): Promise<string[]> {
  if (activeDbProvider === "supabase") {
    return getSupabaseRecruiterNotes(candidateId);
  }
  const store = loadLocalDatabase();
  if (!store.recruiterNotes) {
    store.recruiterNotes = {};
  }
  return store.recruiterNotes[candidateId] || [];
}

async function saveDatabaseRecruiterNote(candidateId: string, note: string): Promise<string[]> {
  if (activeDbProvider === "supabase") {
    return saveSupabaseRecruiterNote(candidateId, note);
  }
  const store = loadLocalDatabase();
  if (!store.recruiterNotes) {
    store.recruiterNotes = {};
  }
  if (!store.recruiterNotes[candidateId]) {
    store.recruiterNotes[candidateId] = [];
  }
  store.recruiterNotes[candidateId].push(note);
  saveLocalDatabase(store);
  return store.recruiterNotes[candidateId];
}

async function getDatabaseEmailLogs(workspaceId: string = "sandbox"): Promise<any[]> {
  if (activeDbProvider === "supabase") {
    return getSupabaseEmailLogs(workspaceId);
  }
  const store = loadLocalDatabase();
  return (store.emailLogs || []).filter(log => {
    if (workspaceId === "sandbox") {
      return !log.workspaceId || log.workspaceId === "sandbox";
    }
    return log.workspaceId === workspaceId;
...
async function saveDatabaseEmailLog(email: any, workspaceId: string = "sandbox"): Promise<void> {
  if (activeDbProvider === "supabase") {
    return saveSupabaseEmailLog(email, workspaceId);
  }
  const store = loadLocalDatabase();
  if (!store.emailLogs) {
    store.emailLogs = [];
  ...
  store.emailLogs.push(dataToSave);
  saveLocalDatabase(store);
}

async function getDatabaseSession(token: string): Promise<any> {
  if (activeDbProvider === "supabase") {
    try {
      const { data, error } = await supabase.from('sessions').select('data').eq('id', token).single();
      if (data) return data.data;
    } catch (err) {
      console.warn("Failed to retrieve session from Supabase, trying local database:", err);
    }
  }
  const store = loadLocalDatabase();
  if (!store.sessions) {
...
async function saveDatabaseSession(token: string, sessionData: any): Promise<void> {
  if (activeDbProvider === "supabase") {
    try {
      ...
      console.warn("Failed to write session in Supabase, trying local database:", err);
    }
  }
  const store = loadLocalDatabase();
  if (!store.sessions) {
...
// Export aliases for backwards compatibility with any remaining code
const getFirestoreCampaigns = getDatabaseCampaigns;
const saveFirestoreCampaign = saveDatabaseCampaign;
const deleteFirestoreCampaign = deleteDatabaseCampaign;
const getFirestoreCandidates = getDatabaseCandidates;
const saveFirestoreCandidate = saveDatabaseCandidate;
const deleteFirestoreCandidate = deleteDatabaseCandidate;
const getFirestoreAuditLogs = getDatabaseAuditLogs;
const saveFirestoreAuditLog = saveDatabaseAuditLog;
const getFirestoreBranding = getDatabaseBranding;
const saveFirestoreBranding = saveDatabaseBranding;
const getFirestoreRecruiterNotes = getDatabaseRecruiterNotes;
const saveFirestoreRecruiterNote = saveDatabaseRecruiterNote;
const getFirestoreEmailLogs = getDatabaseEmailLogs;
const saveFirestoreEmailLog = saveDatabaseEmailLog;
...
app.post("/api/auth/magic-link-send", async (req, res) => {
  try {
    ...
    await saveDatabaseSession(token, {
      email,
      token,
      workspaceId,
...
app.all("/api/auth/magic-link-verify", async (req, res) => {
  try {
    ...
    const sessionData = await getDatabaseSession(token);
    if (!sessionData) {
...
// --- SUPABASE CLIENT CONFIGURATION EXPOSTULATION ---
app.get("/api/supabase-client-config", (req, res) => {
  const currentOrigin = req.headers.origin || `${req.secure ? "https://" : "http://"}${req.headers.host}`;
  res.json({
    supabaseUrl: process.env.SUPABASE_URL || supabaseConfig.supabaseUrl || "",
    supabaseKey: process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY || supabaseConfig.supabaseServiceKey || "",
    appUrl: process.env.APP_URL || currentOrigin
  });
});
...
app.get("/api/db-status", async (req, res) => {
  ...
  const sqlSchema = `-- Copy and paste this script into your Supabase SQL Editor to configure the JSON proxy database:
...
-- 5. Create recruiter_notes table
...
-- Disable Row Level Security (RLS) for dynamic workspace proxies
alter table campaigns disable row level security;
alter table candidates disable row level security;
alter table audit_logs disable row level security;
alter table branding disable row level security;
alter table recruiter_notes disable row level security;
alter table email_logs disable row level security;
`;

  res.json({
    activeDbProvider,
    supabaseUrl: supabaseUrl ? `${supabaseUrl.substring(0, 18)}...` : "",
    supabaseStatus,
    supabaseError,
    sqlSchema
  });
...
app.post("/api/save-supabase-config", async (req, res) => {
  try {
    const { supabaseUrl: urlParam, supabaseServiceKey: keyParam } = req.body;
    if (!urlParam || !keyParam) {
      return res.status(400).json({ error: "Both supabaseUrl and supabaseServiceKey are required configuration fields." });
    }

    const config = {
      supabaseUrl: urlParam.trim(),
      supabaseServiceKey: keyParam.trim()
    };

    const sfPath = path.join(process.cwd(), "supabase-applet-config.json");
    fs.writeFileSync(sfPath, JSON.stringify(config, null, 2), "utf-8");
    ...
    console.log(`Supabase configuration saved & dynamic connection active: ${config.supabaseUrl}`);
    
    res.json({ success: true, message: "Supabase configuration updated. Switched to Supabase mode successfully!" });
  } catch (err: any) {
    res.status(500).json({ error: "Failed to apply Supabase keys: " + err.message });
  }
});

app.post("/api/clear-supabase-config", async (req, res) => {
  try {
    const sfPath = path.join(process.cwd(), "supabase-applet-config.json");
    if (fs.existsSync(sfPath)) {
    ...
    supabaseConfig = {};
    supabase = null;
    activeDbProvider = "local";
    res.json({ success: true, message: "Supabase keys deleted successfully! Dual fallback to local JSON database active." });
  } catch (err: any) {
    res.status(500).json({ error: "Failed to erase Supabase configuration: " + err.message });
  }
});
